...
Базовый `Core\Controller` содержит helper-ы для частых проверок:

- `validate()` - отправляет JSON-ошибку, если условие ложно;
- `positiveId()` - проверяет положительный id и возвращает `?int`;
- `validateStringLength()` - проверяет строку, обязательность и максимальную длину.

Контроллеры используют эти helper-ы для HTTP-валидации входных данных. Проверки существования записей и результата операций пока остаются в контроллерах; позже их можно вынести в service layer.
