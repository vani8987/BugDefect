...
Форма создания дефекта уже содержит поля названия, описания, выбора начального статуса и исполнителя из участников доски.

Карточки дефектов пока остаются frontend-заглушкой через `items`, позже они будут сохраняться в backend и приходить вместе со статусами или отдельным API дефектов.
