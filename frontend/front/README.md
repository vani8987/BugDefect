...
- удаление статуса с промежуточным состоянием `Удаление статуса...`;
...
Форма создания дефекта содержит поля названия, необязательного описания, выбора начального статуса и исполнителя из участников доски.

Дефекты создаются через отдельный `src/stores/defectsStore.ts`, сохраняются на backend и возвращаются внутри `items` каждого статуса. На странице доски карточки показывают код дефекта, название, описание, исполнителя, автора и позицию.

Следующий шаг для рабочей доски - drag-and-drop дефектов между статусами с сохранением нового `status_id` и `position`.
